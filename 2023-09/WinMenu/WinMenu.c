...
void MessageProc_WM_CREATE(HWND hWnd)
{
	...
	//新增弹出菜单项
	InsertMenu(hPopupMenu, -1, MFT_STRING | MFT_OWNERDRAW, ID_VIEW_BIG, TEXT("大图标"));		//弹出菜单内容
	InsertMenu(hPopupMenu, -1, MFT_STRING | MFT_OWNERDRAW, ID_VIEW_SMAL, TEXT("小图标"));		//弹出菜单内容
	InsertMenu(hMainMenu, -1, MFT_STRING | MFT_OWNERDRAW | MF_POPUP, (UINT_PTR)hPopupMenu, TEXT("视图"));	//弹出菜单链接上主菜单

	InsertMenu(hSubMenu, -1, MF_SEPARATOR, NULL, NULL);
	InsertMenu(hSubMenu, -1, MFT_STRING | MFT_OWNERDRAW, ID_EDIT_RED, TEXT("红(&R)"));
	InsertMenu(hSubMenu, -1, MFT_STRING | MFT_OWNERDRAW, ID_EDIT_GREEN, TEXT("绿(&G)"));
	InsertMenu(hSubMenu, -1, MFT_STRING | MFT_OWNERDRAW, ID_EDIT_BLUE, TEXT("蓝(&B)"));

	InsertMenu(hSubMenu, -1, MF_SEPARATOR, NULL, NULL);

	//于弹出菜单项中添加子弹出菜单
	hPopupMenu = CreateMenu();
	InsertMenu(hPopupMenu, -1, MFT_STRING | MFT_OWNERDRAW, ID_EDIT_UPPER, TEXT("大写字母(&U)"));
	InsertMenu(hPopupMenu, -1, MFT_STRING | MFT_OWNERDRAW, ID_EDIT_LOWER, TEXT("小写字母(&L)"));
	InsertMenu(hSubMenu, -1, MFT_STRING | MFT_OWNERDRAW | MF_POPUP, (UINT_PTR)hPopupMenu, TEXT("更改大小写(&N)"));
	...
	InsertMenu(GetSystemMenu(hWnd, FALSE), -1, MFT_STRING | MFT_OWNERDRAW, ID_FILE_OPEN, TEXT("打开(&O)\tCtrl+O"));
...
void MessageProc_WM_DRAWITEM(LPDRAWITEMSTRUCT lpDIS)
{
	TCHAR szTextBuffer[512] = { 0 };
	TCHAR* pos = szTextBuffer;
	LPTSTR lpTextChars = TEXT("\t");
	HBRUSH hBrush = CreateSolidBrush(0x565656);
	wsprintf(szTextBuffer, lpDIS->itemData);
	...
	pos = _ftcstok(szTextBuffer, lpTextChars);
	DrawText(lpDIS->hDC, pos, _tcslen(pos),
		&(lpDIS->rcItem), DT_LEFT | DT_VCENTER | DT_SINGLELINE);
	if (pos = _ftcstok(NULL, lpTextChars))
	{
		DrawText(lpDIS->hDC, pos, _tcslen(pos),
			&(lpDIS->rcItem), DT_RIGHT | DT_VCENTER | DT_SINGLELINE);
	}

	DeleteObject(hBrush);
}

void MessageProc_WM_MEASUREITEM(LPARAM lParam, LPMEASUREITEMSTRUCT* lppMIS)
{
	*lppMIS = (LPMEASUREITEMSTRUCT)lParam;

	//菜单项高宽
	if ((*lppMIS)->itemID == (UINT)-1)
	{
		(*lppMIS)->itemWidth = 50;
		(*lppMIS)->itemHeight = 30;
	}
	//子菜单项高宽
	else
	{
		(*lppMIS)->itemWidth = 150;
		(*lppMIS)->itemHeight = 30;
	}
}
...
LRESULT CALLBACK MainWndProc(HWND hWnd, UINT msgID, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_RBUTTONUP:
		...
		return 0;
	case WM_MEASUREITEM:
		MessageProc_WM_MEASUREITEM(lParam, &lpMIS);
		return TRUE;
	case WM_DRAWITEM:
		MessageProc_WM_DRAWITEM((LPDRAWITEMSTRUCT)lParam);
		return TRUE;
	default:
		return DefWindowProc(hWnd, msgID, wParam, lParam);
	}
